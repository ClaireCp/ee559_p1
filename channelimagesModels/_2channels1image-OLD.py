...
def train_model_2C(model, train_input, train_target, train_classes, test_input, test_target, test_classes, optimizer, mini_batch_size, nb_epochs, verbose=True):
...
def multiple_training_runs(model, nb_runs, optimizer, train_input, train_target, test_input, test_target, mini_batch_size=1000, nb_epochs=300, verbose=True, plot=True):
    ...
    for i in range(nb_runs):
        ...
        model, time_elapsed, best_acc, val_acc_history, test_acc_history = train_model_2C(model, train_input, train_target, test_input, test_target, optimizer, mini_batch_size=mini_batch_size, nb_epochs=nb_epochs, verbose=verbose)
        ...
        acc_test = test_model_2C(model, test_input, test_target)
        list_acc_test.append(acc_test)
        
    if plot == True:
        title = 'Plot for 10 runs with _2channels1image framework (binary cross-entropy loss) and model = {}, \n accuracy obtained during training (model in eval mode) on the training set, and on the test set \n Note: here, the accuracy is for the task "predicting first digit le second digit"'.format(model.name)
